...
use crate::view::{RegisterView, ErrorView};
...
async fn forward_req<'view>(client: &'view Client, dto: RegisterDTO, uri: &'view Uri) -> Result<(),ErrorView<'view>> {
    println!(
        "Forward Request: {} {}",
        uri,
    ...
    if res.is_err() {
        ...
        return Err(ErrorView::new(StatusCode::BAD_GATEWAY.as_u16(),
    ...
    if body.is_err() {
        ...
        return Err(ErrorView::new(StatusCode::INTERNAL_SERVER_ERROR.as_u16(),
    ...
    if StatusCode::BAD_REQUEST == res.status() {
        return Err(ErrorView::new(StatusCode::BAD_REQUEST.as_u16(),
