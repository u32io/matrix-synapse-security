...
use crate::dto::{InviteDTO, RegisterFormDTO, RegisterDTO};
...
pub async fn get_index(invite: web::Query<InviteDTO>, app_state: web::Data<AppState>) -> impl Responder {
    println!("GET /");
...
pub async fn post_index(form: web::Form<RegisterFormDTO>, app_state: web::Data<AppState>, client: web::Data<Client>) -> impl Responder {
    use std::fs;
    println!("POST /");

    match &form.password.eq(&form.re_password) {
        true => {
            let resp = forward_req(&client,
